...
void logger(pid_t pid, int order_size, int to_prepare, int to_send)
{
    ...
    printf("(%d %lds %ldms) Wysłałem zamówienie o wielkości %d(3 * %d). Liczba zamówień do przygotowania: %d. Liczba zamówień do wysłania: %d\n",
           pid, tm.tv_sec, tm.tv_nsec % 1000, 3*order_size, order_size,to_prepare, to_send);
}
...
int main(int argc, char **argv)
{
    ...
    while (TRUE)
    {
        ...
        //if there are no orders to send release memory
        if (orders_ptr->to_send_count == 0){
        ...
        order_size =  orders_ptr->orders[orders_ptr->next_send_idx];
        (orders_ptr->next_send_idx)++;

        orders_ptr->next_send_idx = orders_ptr->next_send_idx % MAX_ORDERS_COUNT;
